...
def unproject(
    ...
    coordinates = homogenize_points(coordinates).float()
    ...
    return ray_directions * z.float()[..., None]
...
def transform_normal_world2cam(
    normals: Float[Tensor, "*#batch dim"],
    extrinsics: Float[Tensor, "*#batch dim+1 dim+1"],
) -> Float[Tensor, "*batch dim"]:
    """Transform normals from 3D world coordinates to 3D camera coordinates."""
    rotation_world2cam = extrinsics[..., :3, :3].inverse()
    outputs = torch.matmul(rotation_world2cam, normals[..., None])
    return torch.nn.functional.normalize(outputs, dim=-2).squeeze(-1)


def transform_normal_cam2world(
    normals: Float[Tensor, "*#batch dim"],
    extrinsics: Float[Tensor, "*#batch dim+1 dim+1"],
) -> Float[Tensor, "*batch dim"]:
    """Transform normals from 3D camera coordinates to 3D world coordinates."""
    rotation_cam2world = extrinsics[..., :3, :3]
    outputs = torch.matmul(rotation_cam2world, normals[..., None])
    return torch.nn.functional.normalize(outputs, dim=-2).squeeze(-1)
