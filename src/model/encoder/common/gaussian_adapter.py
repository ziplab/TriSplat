...
from ....geometry.projection import get_world_rays, homogenize_points
...
class Gaussians:
    ...
    mapped_scales: Float[Tensor, "*batch 3"]
...
class UnifiedGaussianAdapter(GaussianAdapter):
    def forward(
        ...
        extrinsics: Optional[Float[Tensor, "*#batch 4 4"]] = None,
        ...
        mapped_scales = scales
        # scales = 0.005 * F.softplus(scales)
        # scales = scales.clamp_max(3)

        # print(f"max scale: {scales.max().item()}, min scale: {scales.min().item()}, mean scale: {scales.mean().item()}, median scale: {scales.median().item()}")
        ...
        if extrinsics is not None:
            ...
            means = einsum(extrinsics, homogenize_points(means), "... i j, ... j -> ... i")[..., :3]
...
class LocalGaussianAdapter(GaussianAdapter):
    def forward(
        self,
        means: Float[Tensor, "*#batch 3"],
        opacities: Float[Tensor, "*#batch"],
